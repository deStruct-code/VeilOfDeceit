...
type ClientInfo = { ws: WebSocket; playerId: PlayerId };

const rooms = new Map<RoomCode, Map<PlayerId, ClientInfo>>();
const roomSlots = new Map<RoomCode, Map<PlayerId, 'player-1' | 'player-2'>>();
...
app.get('/api/rooms/:code', (req, res) => {
  ...
  res.json({ code, playerCount: room ? room.size : 0, capacity: 2 });
});

app.get('/api/game/:id', async (req, res) => {
  const id = String(req.params.id || '').toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 6);
  try {
...
app.post('/api/game/:id/reset', async (req, res) => {
  const id = String(req.params.id || '').toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 6);
  if (!id || id.length !== 6) { res.status(400).json({ error: 'Invalid game id' }); return; }
  try {
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', async (raw: RawData) => {
    ...
    const slots = roomSlots.get(roomCode) ?? new Map<PlayerId, 'player-1' | 'player-2'>();
    if (!slots.has(playerId)) {
      const taken = new Set(slots.values());
      const slot: 'player-1' | 'player-2' = taken.has('player-1') ? 'player-2' : 'player-1';
    ...
    if (existingRoom.size >= 2) {
      ...
      if (!existing) {
        ...
        await gameRepository.save(createInitialGameState(roomCode, nameFor('player-1'), nameFor('player-2')));
      }
      broadcast(roomCode, { type: 'ready', roomCode, playerCount: existingRoom.size });
    }
  });
  ...
  ws.on('close', () => {
    ...
    if (room.size === 0) {
      ...
      broadcast(joinedRoom, { type: 'joined', roomCode: joinedRoom, playerCount: room.size });
    }
  });
});
