...
const RuneSymbol = ({className}: {className?: string}) => (
    <svg viewBox="0 0 40 40" className={className} fill="none" xmlns="http://www.w3.org/2000/svg" width="28" height="28">
        <path d="M20 4 L20 36 M8 12 L32 12 M8 28 L32 28 M8 12 L20 4 L32 12 M8 28 L20 36 L32 28"
              stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" opacity="0.7"/>
    </svg>
);
            ...
            <div className={styles.card}>
                ...
                {isMeLoading ? (
                    ...
                    <div className={styles.authBlock}>
                        {authError && <p className={styles.authError}>Ошибка авторизации. Попробуй снова.</p>}
                ...
                <div className={styles.section}>
                    ...
                    <div className={styles.nicknameRow}>
                        ...
                        />
                        {me && nickname !== me.name && (
                            <button className={styles.resetNickBtn} onClick={() => { savePlayerName(me.name); setNickname(me.name); setNicknameSaved(true); }}>↺</button>
                        )}
                    </div>
                </div>
                ...
                <div className={styles.buttons}>
                    <button className={styles.btnSolo} onClick={handleSoloGame} disabled={isBusy}>
                ...
                <div className={styles.section}>
                    ...
                    <div className={styles.inputRow}>
                        <input
                            ...
                            maxLength={6}
                        />
                        <button className={styles.btnJoin} disabled={!canJoin} onClick={() => handleEnterRoom(normalizedJoinCode)}>
                            Войти
                        </button>
                    </div>
                    <p className={styles.hint}>Получи код от другого игрока чтобы присоединиться</p>
                </div>

                {/* Footer runes */}
                <div className={styles.runes}>
                    {["ᚠ","ᚢ","ᚦ","ᚨ","ᚱ"].map((r, i) => (
                        <span key={i} className={`${styles.rune} ${i === 2 ? styles.runeMid : styles.runeOuter}`}>{r}</span>
                    ))}
                </div>
